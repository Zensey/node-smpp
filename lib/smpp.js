...
Session.prototype._extractPDUs = function() {
	...
	while (!this.paused) {
		...
		var ref = this._referenceObjects[pdu.sequence_number];
		this.emit('pdu', pdu, ref);
		this.emit(pdu.command, pdu, ref);
		if (pdu.isResponse() && this._callbacks[pdu.sequence_number]) {
			this._callbacks[pdu.sequence_number](pdu, ref);
			delete this._callbacks[pdu.sequence_number];
		}
		if(ref) {
			delete this._referenceObjects[pdu.sequence_number];
		}
	}
	this._busy = false;
};

Session.prototype.send = function(pdu, responseCallback, sendCallback, referenceObj) {
	...
	if (!pdu.isResponse()) {
		...
		if (referenceObj) {
			this._referenceObjects[pdu.sequence_number] = referenceObj;
		}
	} else if (responseCallback && !sendCallback) {
		sendCallback = responseCallback;
	}
	this.socket.write(pdu.toBuffer(), function() {
		this.emit('send', pdu, referenceObj);
		if (sendCallback) {
			sendCallback(pdu, referenceObj);
		}
	}.bind(this));
	return true;
};
...
var createShortcut = function(command) {
	return function(options, responseCallback, sendCallback, referenceObj) {
		var pdu = new PDU(command, options);
		return this.send(pdu, responseCallback, sendCallback, referenceObj);
	};
};
